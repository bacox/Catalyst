...
if __name__ == '__main__':
    ...
    if not args.o:
        ...
        server_learning_rates = [0.1, 0.05, 0.025, 0.01, 0.005]
        # server_learning_rates = [ 0.01]
        num_clients = [50]
        for _r, n, s_lr, model_name in itertools.product(range(repetitions), num_clients, server_learning_rates, model_list):
            exp_id += 1
            configs.append({
                'exp_id': exp_id,
                'aggregation_type': 'async',
                'name': f'afl-{model_name}-{dataset}-n{n}_lr{s_lr}_async',
            ...
            configs.append({
                'exp_id': exp_id,
                'aggregation_type': 'async',
                'name': f'sasgd-{model_name}-{dataset}-n{n}_lr{s_lr}_sasgd',
                'num_rounds': num_rounds,
                'clients': {
                        'client': AFL.Client,
                        'client_args': {
