from asyncfl.server import Server, fed_avg, get_update, no_defense_update, parameters_dict_to_vector_flt
import math
import numpy as np
import logging
...
class FedWait(Server):
    ...
    def client_weight_update(self, client_id, weights: dict, gradient_age: int, is_byzantine: bool): 
        ...
        logging.info(f'Adding weights of client {client_id}')
        self.waiting_models[client_id] = weights
        if len(self.waiting_models) == self.n:
            logging.info('Aggregate')
            new_params = fed_avg(self.waiting_models)
