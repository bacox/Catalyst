...
class SemiAsync(Server):
    ...
    def client_weight_dict_vec_update(self, client_id: int, weight_vec: np.ndarray, gradient_age: int, is_byzantine: bool) -> np.ndarray:
        ...
        logging.debug(f'{len(self.sched_ctx.clients_adm["idle"])=}')
        self.idle_clients_count.append(len(self.sched_ctx.clients_adm["idle"]))

        logging.info(f'Overview of idle clients: {self.idle_clients_count}')
