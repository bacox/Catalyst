...
class ValidateServer(Server):
    """
    The main idea is to build 2f+1 validation for each update
    """
    ...
    def client_weight_dict_vec_update(self, client_id: int, weight_vec: np.ndarray, gradient_age: int, is_byzantine: bool) -> np.ndarray:
        ...
        if np.isnan(weight_vec).any():
            ...
            if len(self.model_full_history) >= max(self.f * 2 + 1, 2):
                ...
                args_dict = {'num_users': self.n, 'frac': 1.0, 'malicious': self.f, 'wrong_mal': 0, 'right_ben': 0, 'turn':0}
                ...
                logging.info(f'[BYZANTINE] Update is accepted ? {accepted} and is byzantine ? {is_byzantine}')
