...
if __name__ == "__main__":
    ...
    if not args.o:
        ...
        servers = [
            [AFL.PessimisticServer, {"learning_rate": server_lr, "k": 3, "disable_alpha": False}, 'semi-async'],
        ]
        f0_keys = []

        generated_ct = {}

        for _r, server, var_set, atk, val_f in itertools.product(range(repetitions), servers, var_sets, attacks, preset_f):
            ...
            key_name = f'f{val_f}_n{num_clients}_lr{server_lr}_{model_name.replace("-", "_")}_{server_name}'
    ...
    for running_stats, cfg_data in outputs2:
        ...
        local_df_name = f"f{f}-{server_lr}-{name_suffix}"
