...
if __name__ == '__main__':
    ...
    print('Exp 39: Pessimistic Server: effect of the staleness function for the delayed updates')

    (data_path := Path('.data')).mkdir(exist_ok=True, parents=True)
    (graphs_path := Path('graphs')).mkdir(exist_ok=True, parents=True)
    exp_name = 'exp39_pess_staleness_effect_25p'
    ...
    if not args.o:
        ...
        servers = [
            [AFL.PessimisticServer,{'learning_rate': 0.1, 'k': 3, 'disable_alpha': True}],
            [AFL.PessimisticServer,{'learning_rate': 0.1, 'k': 3, 'disable_alpha': False}],
            [AFL.PessimisticServer,{'learning_rate': 0.1, 'k': 3, 'aggregation_bound': 40, 'disable_alpha': False}],
        ]
        f0_keys = []
        ...
        for _r, server, var_set, atk in itertools.product(range(repetitions), servers, var_sets, attacks):
            ...
            if ct_key not in generated_ct.keys():
                ct_clients = np.abs(np.random.normal(100, 60.0, num_clients - f))
                f_ct = np.abs(np.random.normal(100, 60.0, f))
    ...
    for running_stats, cfg_data in outputs2:
        ...
        local_df_name = f'{parts[-2]}-f{f}-{server_lr}-{kardam_damp}-{name_suffix}-da-{disable_alpha}'
    ...
    graph_file = graphs_path / f'{exp_name}_rounds.png'
    ...
    merged = pd.merge(left=server_df, right=interaction_events_df, on=['round', 'name'], how='left')
    ...
    for idx, row in  inspect.iterrows():
        ...
        print(f'[{idx} | {row["client_id"]}] {row["round"]} ::{row["accuracy"]} :: {row["wall_time"]} :: {row["name"]} :: {row["client_ct"]}')
