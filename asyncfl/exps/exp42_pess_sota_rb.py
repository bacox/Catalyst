...
if __name__ == "__main__":
    ...
    print("Exp 42: Pessimistic Server: compare against the state of the art against random perbutation")

    (data_path := Path(".data")).mkdir(exist_ok=True, parents=True)
    (graphs_path := Path("graphs")).mkdir(exist_ok=True, parents=True)
    exp_name = "exp42_pess_sota_byz_RD"
    ...
    if not args.o:
        ...
        servers = [
            ...
            # [AFL.Server,{'learning_rate': server_lr}],
            [AFL.BASGD,{'learning_rate': server_lr, 'num_buffers': 20},'async'],
