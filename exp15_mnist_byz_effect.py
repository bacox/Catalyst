...
if __name__ == '__main__':
    ...
    print('Exp 15: Show the effect of byzantine nodes on a async system. Number of nodes used 50 with 3 and 6 byzantine nodes. Attacks: Negative gradient (magnitude=10) and random perbutation (alpha_atk = 0.2). Learning rate follows from learning rate sweep: exp 14')

    (data_path := Path('.data')).mkdir(exist_ok=True, parents=True)
    (graphs_path := Path('graphs')).mkdir(exist_ok=True, parents=True)
    exp_name = 'exp15_mnist_byzantine_effects'
